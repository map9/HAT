...
import { ChatMessage, ChatRoleType, ETLInnerChat, ETLInnerChatMessage, EtlResponseObject } from '../ts/LLMHelper';

import ETLMessage from './ETLMessage.vue';
...
const editableInputDiv = ref<HTMLElement | null>(null);
const chatViewDiv = ref<HTMLElement | null>(null);
...
onMounted(() => {
  if (editableInputDiv.value)
    editableInputDiv.value.focus();
});

function minimizeComponent() {
  if (!editableInputDiv.value?.contains(document.activeElement)) {
...
const updateMessage = (event: Event) => {
  ...
  //userInput.value = editableInputDiv.value? editableInputDiv.value.innerText : '';
};
...
function getETLAgentInnerChatSets(message): ETLInnerChat[] {
  let innerChatSets: ETLInnerChat[] = message.getInnerChatSets();
  // 强制VUE刷新模版，VUE好像并不好用。
  innerChatSets = [...innerChatSets];
  return innerChatSets;
}

function getETLAgentResultMessage(message): ETLInnerChatMessage | undefined {
  //let innerChatMessage: ETLInnerChatMessage | undefined = message.getResultMessage();
  //innerChatMessage = innerChatMessage? { ...innerChatMessage } : undefined;
  //return innerChatMessage;
...
function sendMessage() {
  ...
  userInput.value = '';
  if (editableInputDiv.value)
    editableInputDiv.value.innerText = '';
  ...
  if (message.content.startsWith('@etl_agent') == true){
    responseMessage.role = ChatRoleType.ETL_AGENT;    
    eventSource = new EventSource(`/llm/event/etl?p=${encodeURIComponent(message.content.slice(10))}`);
    receiveETLAgentResponse(eventSource);
  } else {
    responseMessage.role = ChatRoleType.ASSISTANT;
    eventSource = new EventSource(`/llm/event/assistant?p=${encodeURIComponent(message.content)}`);
    receiveAssistantResponse(eventSource);
  }
}

function praseSSEData(sseData, callback) {
  sseData = sseData.replaceAll("&nbsp;", " ").replaceAll("&#10;", "\n")
  //console.log(sseData);
...
function receiveAssistantResponse(eventSource) {
  let responseBuffer = "";
  eventSource.onmessage = async (event) => {
    ...
    chatMessages.value[chatMessages.value.length - 1].content = responseBuffer;
    await nextTick();
    scrollToBottom();
  };
...
function receiveETLAgentResponse(eventSource) {
...
function scrollToBottom() {
  if (chatViewDiv.value) {
    chatViewDiv.value.scrollTo({ top: chatViewDiv.value.scrollHeight, behavior: 'smooth' });
  }
}

const showCopy = (event) => {
  const copyIcon = event.currentTarget.querySelector('.copy-icon');
  if (copyIcon) {
...
const OnCopy = async (message: ChatMessage, index) => {
  try {
    if (message.role == ChatRoleType.USER || message.role == ChatRoleType.ASSISTANT){
      await navigator.clipboard.writeText(message.content as string);
    } else {
      var etlResponseObject: EtlResponseObject = message.content as EtlResponseObject;
      var resultMessage = etlResponseObject.getResultMessage();
      if (resultMessage != undefined){
        await navigator.clipboard.writeText(JSON.stringify(resultMessage.message, null, 2));
      } else {
        toast.error(`没有提取到结果，无法复制。`);
      }
    }
...
.chat-message.user .body .content {
  padding: 10px 10px;
  background-color: var(--surface-gray-50);
  border-radius: 8px;
}

.chat-message.assistant .body .content {
  padding: 0px 0px 10px 10px;
}

.chat-message.etl-agent .body .content {
  padding: 0px 0px 10px 10px;
  border-radius: 8px 8px 0px 0px;
}

:deep(.chat-message .body .content p) {
...
.input-view-container:hover {
  border: 1px solid transparent;
  box-shadow: 0 4px 12px var(--shadow-16);
  border-color: var(--border-black-8);
}

.input-view-container:focus-within {
  border: 1px solid transparent;
  box-shadow: 0 2px 8px 1px var(--shadow-16);
}

.input-view-container.minimized {
  cursor: pointer;
  justify-content: center;
  opacity: 0.8;
...
.input-view-container:not(.minimized) {
  opacity: 1;
}
